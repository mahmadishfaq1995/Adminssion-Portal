...
Class testslipModel extends CI_Model {
    ...
    function information($id) {
        $query = "SELECT p.email,p.name,p.father_name,p.district,p.address,p.image,p.gender,r.roll_no,p.cnic FROM personal_info as p inner join regsiter as r on p.register_id=r.register_id where p.register_id= " . $id . "";

        $result = $this->db->query($query);
        return $result->result();
    }
	function informatio_seating($roll_no){
