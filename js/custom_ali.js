			   ...
			   $.ajax({
				...
				success: function (data) {
                                    if(data == 'Done'){
                                      ...
                                      alert("Username OR Email OR CNIC already exists try another!!");
                                      return false;
                                  }
				}
				});
			   ...
			   $.ajax({
				...
				success: function (data) {
                                    ...
                                    if(data == 'done')
                                    {
										...
										alert("Please Refill the form and Enter all fields Carefully, Only Phone No is Optional...");
										return false;
									}
                                }
				});
                  ...
                  $("#save_acadamic_data").click(function(){
                    var result = {
                                  ...
                                  'itotal1': $("#total_inter1").val(),
                                  'imarks1': $("#marks_inter1").val(),
                                  'imajor1': $("#major_inter2").val(),
                                  'iboard1': $("#board_inter2").val(),
                                  'iyear2': $("#year_inter2").val(),
                                  'iroll2': $("#rollno_inter2").val(),
                                  'itotal2': $("#total_inter2").val(),
                                  'imarks2': $("#marks_inter2").val(),
				...
				success: function (data) {
                                    if(data == 'done')
                                    {
                                      ...
                                      window.close(this);
										...
										alert("Please Fill All Required Fields, Only Inter Part I is Optional....");
										return false;
									}
                                }
                            });
    ...
    $("#issue_slip").click(function(){
         $.ajax({
                ...
                success: function (data) {
                    if(data == 'not_issued'){
                        $("#m_head").html("Alert!!!");
                        $("#m_body").html('Your Challan is not verified yet.. Wait for a few working days');
                        $("#myModal").modal("show");
                        $("#btn_suc").hide();
                    }
                    else if(data == 'done')
...
function save_edit_pinfo()
{
    var result = {
                'name': $("#fullname").val(),
                'dob': $("#dob").val(),
                'gender': $('input:radio[name=gendere]:checked').val(),
                'cnic': $("#cnic1").val(),
                'fname': $("#fname").val(),
                'fcnic': $("#fcnic1").val(),
                'district': $("#district").val(),
...
function username_field(field) {
    var key = window.event.charCode;
    key = String.fromCharCode(key);
    if ((!key.match(/[a-z]/i) && key!='_' && key!=' ' && !key.match(/[0-9]/i))){
		var index="#"+field;
	    $(index).show();
		event.returnValue = false;
    }
...
function readURLE(input) {
    if (input.files && input.files[0]) {
        ...
        reader.readAsDataURL(input.files[0]);
		var file_data = $('#picture_submit').prop('files')[0];   
                    var form_data = new FormData();                  
                    form_data.append('file', file_data);           
                    $.ajax({
                              url: 'printchallan/store_picture', // point to server-side PHP script 
                                dataType: 'text',  // what to expect back from the PHP script, if anything
                                cache: false,
                                contentType: false,
                                processData: false,
...
$("#img_upload").change(function(){
	...
	$("#picture_submit").change(function(){
        readURLE(this);
    });
