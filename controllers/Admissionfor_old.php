...
class admissionfor extends CI_Controller {
    ...
    function store_data() {
        ...
        $data = array('reserve' => $this->input->get('reserve'),
            'name' => $this->input->get('name'),
            'dob' => date("Y/m/d", $this->input->get('dob')),
    ...
    function store_pic() {
        if (0 < $_FILES['file']['error']) {
            ...
            move_uploaded_file($_FILES['file']['tmp_name'], $_SERVER['CONTEXT_DOCUMENT_ROOT'] . '/DCS/img/user_images/' . $this->session->userdata('id') . '.jpg');
        }
    }
